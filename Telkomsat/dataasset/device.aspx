<%@ Page Title="" Language="C#" MasterPageFile="~/ASSET.Master" AutoEventWireup="true" CodeBehind="device.aspx.cs" Inherits="Telkomsat.dataasset.device" %>
...
<script src="../assets/mylibrary/sweetalert.min.js"></script>
    ...
    <script type="text/javascript">
        ...
        $.ajax({
            type: "POST",
            url: "device.aspx/GetID",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                var customers = response.d;
                $('#<%=so2.ClientID %>').empty();
                $('#<%=so2.ClientID %>').append('<option>' + '--Pilih Equipment--' + '</option>');
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $(data).each(function () {
                        ...
                        $('#<%=txtwiayah.ClientID %>').val(this.devices);
                        $('#<%=lblid.ClientID %>').html(this.iddevices);
                        $('#<%=txtid.ClientID %>').val(this.iddevices);
                    });
